...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		addContentView(overlay, new LayoutParams(LayoutParams.MATCH_PARENT,
		...
		resolutionBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				final List<Size> supportedPictureSize = camera.getParameters()
						.getSupportedPictureSizes();
				List<Size> supportedPreviewSize = camera.getParameters()
		...
		INOUTBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (numberOfCameras == 1) {
					Toast.makeText(MainActivity.this, "Cameraが一つです",
							Toast.LENGTH_SHORT).show();
					return;
				...
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					if (camera != null) {
						if (!mIsTake) {
							...
							Toast.makeText(MainActivity.this, "撮影",
									Toast.LENGTH_SHORT).show();
							mIsTake = true;
							// オートフォーカス
	...
	private Camera.ShutterCallback shutterListener = new Camera.ShutterCallback() {
		public void onShutter() {
			if (camera != null) {
				if (!mIsTake) {
					Toast.makeText(MainActivity.this, "撮影", Toast.LENGTH_SHORT)
							.show();
					mIsTake = true;
					// オートフォーカス
					camera.autoFocus(mAutoFocusListener);
	...
	private Camera.PictureCallback pictureListener = new Camera.PictureCallback() {
		public void onPictureTaken(byte[] data, Camera camera) {
			...
			try {
				...
				// 空のイメージを作成
				Bitmap offBitmap = Bitmap.createBitmap(cameraBitmap.getWidth(),
						cameraBitmap.getHeight(), Bitmap.Config.ARGB_8888);

				Canvas offScreen = new Canvas(offBitmap);

				// 画像の合成処理
				offScreen.drawBitmap(cameraBitmap, null, new Rect(0, 0,
						cameraBitmap.getWidth(), cameraBitmap.getHeight()),
						null);
				offScreen.drawBitmap(overlayBitmap, null, new Rect(0, 0,
						cameraBitmap.getWidth(), cameraBitmap.getHeight()),
						null);
				...
				String imgPath = FOLDER_PATH + File.separator
						+ sf.format(cal.getTime()) + ".jpg";
				...
				Toast.makeText(MainActivity.this, "このサイズでは撮影できません,",
						Toast.LENGTH_SHORT).show();
			}
	...
	private boolean createFolder() {
		...
		if (!isSdCardMounted(status)) {
			Toast.makeText(this, "SDカードがマウントされていません", Toast.LENGTH_SHORT)
					.show();
			return false;
		}
